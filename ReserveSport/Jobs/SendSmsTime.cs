    ...
    public class SendSmsTime : IJob
    {
        public Task Execute(IJobExecutionContext context)
        {
            ...
            using (DataBaseContext _ctx = new DataBaseContext(option.Options))
            {
                ...
                var package = _ctx.SmsRemember.Find(1);
                ...
                {
                    TomorrowSmsViewModel tomorrow = new TomorrowSmsViewModel();
                    tomorrow.UserNumber = item.Order.User.PhoneNumber;
                    tomorrow.Start = item.ReserveModel.StartTime;
                    tomorrow.Time = item.ReserveModel.DayTime.ToShamsi();
                    tomorrow.Name = item.Order.User.UserName;
                    tomorrow.Place = item.ReserveModel.Collection.CollectionName;
                    var check = SmsSender.TomorrowSms(tomorrow,sender,package);
                }
            }
